...
@Service
public class WebChatMsgService  {
    ...
    public WebChatRoomDto createChatRoom(String roomName, String roomPwd, boolean secretChk, int maxUserCnt) {
        ...
        // room.setChatType(WebChatRoomDto.ChatType.MSG);
        // 화상 서비스, 문자 서비스를 구분하지 않고
        // 현재 문자 서비스만 등록.
    ...
    public String addUser(Map<String, WebChatRoomDto> chatRoomMap, String roomId, String userName){
        ...
        //회원가입 시 아이디 및 닉네임 중복 사용이 불가능하기 때문에
        //addUser 메소드에서 중복확인 할 필요가 없다.

        //hashmap에서 concurrentHashMap 으로 변경
    ...
    //채팅방 유저 이름 중복 확인
    //회원가입 시 아이디 및 닉네임 중복 사용이 불가능하기 때문에
    //isDuplicateName 메소드에서 중복확인 할 필요가 없다.
    //또한 로그인을 하지 않았을 경우에 채팅이 불가능함으로
    //유저네임이 익명이거나 랜덤으로 부여된 숫자일 수 없다.
    /*
    public String isDuplicateName(Map<String, WebChatRoomDto> chatRoomMap, String roomId, String username){
        WebChatRoomDto room = chatRoomMap.get(roomId);
        String tmp = username;
    ...
    */
    ...
    // 채팅방 특정 유저 삭제
    public void delUser(Map<String, WebChatRoomDto> chatRoomMap, String roomId, String userUUID){
        WebChatRoomDto room = chatRoomMap.get(roomId);
